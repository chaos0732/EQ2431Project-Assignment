...
class DiscreteD:
    ...
    def rand(self, nData):
        ...
        R= row vector with integer random data drawn from the DiscreteD object
           (size(R)= [1, nData]
        """
        R = np.zeros(nData, dtype=int)
        for i in range(nData):
            # draw random number
            r = np.random.rand()
            # find the first element of ProbMass that is larger than r
            # and return its index
            for j in range(len(self.probMass)):
                if r < self.probMass[j]:
                    R[i] = j
                    break
                else:
                    r -= self.probMass[j]
        # if no element was found, return the last index
        if j == len(self.probMass)-1:
            R[i] = len(self.probMass)-1
